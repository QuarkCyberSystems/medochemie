...
def register_batch_hooks():
    ...
    #AttributeError: 'MariaDBDatabase' object has no attribute 'after_save'
...
@frappe.whitelist()
def validate_batch(work_order=None) :
    ...
    try :
        ...
        SELECT
            ...
            sed.serial_and_batch_bundle AS `Batch Bundle`,
            sbe.warehouse AS Wrehouse
        FROM
            `tabStock Entry` se
        JOIN
            `tabStock Entry Detail` sed ON se.name = sed.parent
        JOIN 
            `tabSerial and Batch Bundle` sbb ON sed.serial_and_batch_bundle = sbb.name
        JOIN 
...
@frappe.whitelist()
def get_narcotic_approver_users():
    users = frappe.get_all("Has Role", filters={"role": "Narcotic Approver"}, fields=["parent as user"])
